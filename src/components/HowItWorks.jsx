      ...
      <div className="grid grid-cols-3 gap-10 text-left">
        {steps.map((step, index) => (
          <div key={step.title} className="relative">
            <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-[#F2643B] text-white text-xl mb-5">
              {step.icon}
            </div>
            <span className="absolute top-0 right-0 text-6xl font-bold text-slate-100 -z-10">
              {index + 1}
            </span>
